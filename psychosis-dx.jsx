...
// Scoring
// ============================================================
function scoreAll(activeFeatures) {
  const dxScores = Object.fromEntries(DIAGNOSES.map((d) => [d.id, d.base]));
  const dxContrib = Object.fromEntries(DIAGNOSES.map((d) => [d.id, []]));
  const triggeredFlags = new Set();
  const ruledIn = new Set();
    ...
    if (f.weights) {
      for (const [dx, w] of Object.entries(f.weights)) {
        if (dxScores[dx] !== undefined) {
          dxScores[dx] += w;
          dxContrib[dx].push({ feature: f.label, weight: w });
        }
      }
    }
...
export default function PsychosisDx() {
  ...
  const [openDx, setOpenDx] = useState(null);
  ...
  const activeFeatures = useMemo(
    () => allFeatures.filter((f) => active.has(f.id)),
    [active, allFeatures]
  );
